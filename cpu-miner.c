...
static bool gbt_work_decode(const json_t *val, struct work *work)
{
	...
	if (tmp && json_is_array(tmp)) {
		...
		for (i = 0; i < n; i++) {
			...
			if (!strcmp(s, "segwit") || !strcmp(s, "!segwit"))
				segwit = true;
			if (!strcmp(s, "mweb") || !strcmp(s, "!mweb"))
				mweb = true;
		}
	}
	...
	tmp = json_object_get(val, "seedhash");
	if (!tmp || !json_is_string(tmp)) {
		applog(LOG_ERR, "JSON invalid seedhash");
		goto out;
	}
	hex2bin(((uint8_t*)work->seedhash), json_string_value(tmp), 32);
	...
	if (mweb) {
		work->mweb = strdup(json_string_value(json_object_get(val, "mweb")));
	}
...
static bool submit_upstream_work(CURL *curl, struct work *work)
{
	...
	if (work->txs) {
		...
		if (work->workid) {
			...
			if (work->mweb) {
				req = malloc(128 + 2*80 + strlen(work->txs) + strlen(work->mweb));
				sprintf(req,
					"{\"method\": \"submitblock\", \"params\": [\"%s%s01%s\"], \"id\":1}\r\n",
					data_str, work->txs, work->mweb);
			} else {
...
static void on_new_work(struct work* work) {
	if (have_progpow) {
		if (work->height%2 == 0) {
			uint32_t header[32];
			for (int i=0; i<32; ++i)
				be32enc(&header[i], work->data[i]);
			progminer_set_work(header, work->height, (uint8_t*)work->target);
		} else {
			progminer_pause();
		}
	}
	randomx_set_seedhash((const char*)work->seedhash);
}
...
void progminer_solution_found(uint32_t nonce, int height) {
	struct work wc;
	...
	wc.data[19] = be32dec(&nonce);

	if (!submit_work(NULL, &wc)) {
		applog(LOG_WARNING, "Failed to submit work from progminer to workio thread");
	}
}
...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
		...
		pthread_mutex_lock(&g_work_lock);
		if (time(NULL) - g_work_time >= min_scantime || work.data[19] >= end_nonce) {
			work_free(&g_work);
		...
		if (!rx_vm) {
			rx_vm = randomx_init_worker();
			if (!rx_vm) {
				applog(LOG_ERR, "RandomX VM creation failed");
		...
		if (work.height%2 == 0) { // ProgPoW
			for (uint32_t i = 0; i<(have_longpoll ? LP_SCANTIME : opt_scantime)*100; ++i) {
				usleep(10000); // 10ms
		...
		for (int i=0; i<32; ++i)
			be32enc(&header[i], work.data[i]);
		header[19] = work.data[19];

		hashes_done = 0;
		gettimeofday(&tv_start, NULL);

		/* scan nonces for a proof-of-work hash */
		rc = randomx_scanhash(thr_id, rx_vm, header, work.target, max_nonce, &hashes_done);
		...
		if (rc && !opt_benchmark) {
			work.data[19] = be32dec(&header[19]);
...
int main(int argc, char *argv[])
{
	...
	void* progminer_settings = progminer_setup_devices();
	if (progminer_settings) {
		have_progpow = true;
	} else {
		applog(LOG_WARNING, "No GPU selected. Disabling ProgPoW minig");
	}
	...
	if (have_progpow)
		progminer_start(progminer_settings, &progminer_solution_found);
	...
	applog(LOG_INFO, "%d RandomX miner threads started.", opt_n_threads);
