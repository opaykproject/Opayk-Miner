...
Result EthashAux::eval(int epoch, h256 const& _headerHash, uint32_t _nonce) noexcept
{
...
Result EthashAux::eval(int epoch, int _block_number, h256 const& _headerHash, uint32_t _nonce) noexcept
{
    auto headerHash = ethash::hash256_from_bytes(_headerHash.data());
    auto& context = ethash::get_global_epoch_context(epoch);
